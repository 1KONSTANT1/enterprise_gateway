...
## ssh key for accessing remote node
#  Default: '~/.ssh/id_rsa'
c.EnterpriseGatewayConfigMixin.ssh_key_filename = "/home/michman/.ssh/jupyterhub_slurm"
...
## ssh key for accessing remote node
#  See also: EnterpriseGatewayConfigMixin.ssh_key_filename
# c.EnterpriseGatewayApp.ssh_key_filename = '~/.ssh/id_rsa'
...
#------------------------------------------------------------------------------
# SlurmProxyConfig(Configurable) configuration
#------------------------------------------------------------------------------
...
## The file system path to Enroot container images stored on the Slurm worker
#  nodes.
#  Default: '/mnt'
c.SlurmProxyConfig.enroot_image_path = '/home/michman/dockerf/jupyter-kernel-312.sqsh'

## List of master nodes for Slurm. These nodes will be used as the primary
#  contact points for Slurm job submission.
#  Default: []
c.SlurmProxyConfig.master_nodes = ['michmanc3ab4d15-3d62-4d03-879d-011ddbd7f6f3.novalocal']

## The IP address or hostname of the NFS server that contains user home
#  directories or shared data volumes. This server will be mounted on Slurm
#  worker nodes to provide access to user files, kernelspecs, and shared data
#  across the cluster. The NFS export should be properly configured to allow
#  access from all worker nodes in the Slurm partition.
#  
#  Example: '192.168.1.100' or 'nfs-server.cluster.local'
#  Default: '0.0.0.0'
#c.SlurmProxyConfig.nfs_server = '10.100.203.132'
c.SlurmProxyConfig.nfs_server = '10.100.192.16'

## NFS server user's directory path destination The remote path on the NFS server
#  that contains user directories or shared data. This path corresponds to the
#  NFS export that will be mounted on worker nodes.
#  
#  This is typically the exported path on the NFS server, such as '/home' or
#  '/exports/users'. The kernel processes will access user data through this
#  mount point.
#  
#  Example: '/exports/users' or '/shared_data'
#  Default: '/data'
c.SlurmProxyConfig.nfs_server_dest = '/data'

## The local mount point on each Slurm worker node where the NFS share will be
#  mounted.
#  
#  This directory must exist and be accessible on all worker nodes. The NFS
#  export from the server will be mounted here, making user files available to
#  kernel processes. Ensure this directory has appropriate permissions for the
#  users running kernels.
#  
#  Example: '/mnt/nfs/users' or '/shared'
#  Default: '/mnt/users'
c.SlurmProxyConfig.worker_mount_dir = '/home/michman/nfs_data'

c.SlurmProxyConfig.slurm_pending_timeout = 600.0
